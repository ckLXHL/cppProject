...
template <class T> class my_queue {
	...
	public:
		...
		my_queue(const my_queue<T>& q): head(0), tail(0) { copy_ele(q); };
		template <class it> 
...
void my_queue<T>::assign(it beg, it end){
	destroy();
	copy_ele(beg, end);
}
template <class T> template <class it>
void my_queue<T>::copy_ele(it beg, it end){
	while(beg != end){
		push(*beg);
		beg++;
	}
...
template <class T>
my_queue<T>& my_queue<T>::operator = (const my_queue<T>& Q){
	destroy();
	copy_ele(Q);
}
template <class T>
ostream& operator << (ostream& os, const my_queue<T>& q) {
	os << "<";
	my_queueItem<T> * p;
...
template <class T> 
void my_queue<T>::copy_ele(const my_queue& q){
...
template <class T> 
void my_queue<T>::push(const T& t){
...
template <class T>
ostream& my_queue<T>::print(ostream& os){
	my_queueItem<T> * pt = head;
	while(pt != tail){
		os << pt->item << endl;
		pt = pt->next;
